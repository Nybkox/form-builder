...
import { ArrowDown, InputContainer, Option, OptionsContainer, Value } from './styled';

const Select = forwardRef(
  ({ id, name, error, label, items, possibleItems, onBlur, setValue, value, multiple }, ref) => {
    ...
    const visibleValue = useMemo(
      () =>
        multiple
          ? (possibleItems ?? items).filter((i) => convertToArray(value).includes(i)).join(', ')
          : (possibleItems ?? items).find((i) => i.value === value)?.label,
      [items, multiple, possibleItems, value]
    );
    ...
    return (
      <FieldRoot>
        ...
        <InputContainer>
          <HiddenInput aria-hidden="true" name={name} id={id || name} ref={ref} value={value} onChange={noop} />
